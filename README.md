...
## Install
`git clone https://github.com/reviewbasetg/reviewbase-explorer.git` - copy repo to local folder.

`cd reviewbase-explorer` - change into project directory.
...
```
*/1 * * * * cd /path/to/reviewbase-explorer && ./script/cron_block.sh >> ./tmp/block.log 2>&1
*/1 * * * * cd /path/to/reviewbase-explorer && /path/to/node ./cron/masternode.js >> ./tmp/masternode.log 2>&1
*/1 * * * * cd /path/to/reviewbase-explorer && /path/to/node ./cron/peer.js >> ./tmp/peer.log 2>&1
*/1 * * * * cd /path/to/reviewbase-explorer && /path/to/node ./cron/rich.js >> ./tmp/rich.log 2>&1
*/5 * * * * cd /path/to/reviewbase-explorer && /path/to/node ./cron/coin.js >> ./tmp/coin.log 2>&1
```
